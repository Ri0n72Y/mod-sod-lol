...
- MSBuild / Visual Studio with .NET Framework 4.8.1 support.
- `ShapeOfDreamsHome` points to the Shape of Dreams installation directory.

Example PowerShell session:

```powershell
$env:ShapeOfDreamsHome = "D:\SteamLibrary\steamapps\common\Shape of Dreams"
msbuild .\SodLolCaitlyn.csproj /p:Configuration=Release
```

For easiest iteration, clone/copy the repository into a subfolder under `<ShapeOfDreamsHome>\Mods` so the built assembly and `about/metadata.json` can be loaded by the in-game Mod Manager.
...
| P0-07 | Start a run with Headshot | Caitlyn Q/W/E/R type names are added to the authoritative server loot pool. |
| P0-08 | Obtain one custom active Memory | Resource resolves to the correct custom `SkillTrigger` subclass and can be picked/equipped without a missing-resource error. |
...
| P0-15 | With Headshot equipped, land Lacerta basic attacks | Only main `DoBasicAttackHit` calls advance the synced counter; every fifth qualifying call produces one server bonus-damage event and then resets the counter. |
...
| P0-19 | Compare displayed crit chance with Headshot debug log | `critRaw` matches the game's normalized runtime stat convention; `critRatio` equals the same value clamped to `0..1`. |
...
| P0-22 | Host + one client and obtain/cast a runtime Memory | Mirror runtime prefab spawns correctly, or load fails loudly with a concrete missing private API instead of silently creating a broken network object. |
...
## Static invariants introduced by this revision

Before an in-game run is considered meaningful, the code now enforces these invariants at startup:
...
- missing Mirror private members required by the runtime-prefab workaround abort registration instead of being ignored;
...
- for Headshot tests, `critRaw`, `critRatio`, counter value and whether the hit was main/secondary.
